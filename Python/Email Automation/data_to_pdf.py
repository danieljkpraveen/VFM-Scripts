...
class MinimalPDF:
    ...
    def generate_default_metadata(self):
        ...
        :example: 
        {
            ...
            "ModifyDate": "2023-10-01T12:00:00",
            "PageCount": 0,
            "PDFVersion": "1.4",
            "CustomMetadata": "No",
            "Tagged": "No",
    ...
    def cell(self, w, h, txt):
        ...
        for line in wrapped_text:
            # Adjust text position with padding and move y down by the leading height
    ...
    def output(self):
        ...
        """
        # Ensure the last page content is added before generating the PDF
        if self.current_content:
            self.pages.append(self.current_content)
        # Create a binary stream to hold the PDF content
        pdf = io.BytesIO()
        # Write the PDF header and version
        pdf.write(b"%PDF-1.4\n")
        # Initialize the xref table and object count
        xref = []
        obj_count = 1

        # pdf.tell() points to the current position in the stream which is the data written
        # so far and the position from which the next object will be written.
        # Add current position to xref table
        xref.append(pdf.tell())
        # prepare the PDF info object with metadata
        info_obj = obj_count
